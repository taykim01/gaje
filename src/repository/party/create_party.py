...
def get_dates_in_range(start_date: str, end_date: str) -> List[Dict[str, Any]]:
    dates = []
...
@router.get("/create-party")
async def createParty(name: str, start_date: str, end_date: str) -> Response:
    try:
        ...
        if response.success:
            return Response(True, "DB에 파티를 생성하는데 성공했습니다").to_dict()
        else:
            return Response(False, "DB에 파티를 생성하는데 실패했습니다.").to_dict()
    except:
        return Response(False, "파티 생성에 실패했습니다.", {}).to_dict()